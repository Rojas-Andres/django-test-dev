...
Swagger/OpenAPI y colección Bruno.
...
## Calidad de código (pre-commit + ruff)

El formateo y el linting se hacen con **ruff** (`ruff format` + `ruff check`,
reemplaza a black/isort) más pylint y djLint, orquestados con **pre-commit**.
La configuración de ruff está en [`pyproject.toml`](pyproject.toml)
(`line-length = 79`).

```bash
pip install pre-commit          # ya viene en requirements/development.txt
pre-commit install              # instala el git hook (se ejecuta en cada commit)
pre-commit run --all-files      # ejecutar todos los hooks sobre todo el repo

make ruff                       # solo formatear + lint con autofix
make dev-check                  # atajo de `pre-commit run --all-files`
```
